...
describe("council.define_personas overrides", () => {
    it("applies and persists overrides", async () => {
        ...
        expect(first.personas.find((p) => p.name === "Growth Strategist")?.focus).toContain("product-led growth");
        // second call should read persisted override without providing overrides again
        const second = await tool.handler({});
        expect(second.personas.find((p) => p.name === "Growth Strategist")?.focus).toContain("product-led growth");
    });
